...
# powerbash13k theme for oh-my-bash, inspired by powerlevel13k zsh theme
# made by awerebea, based on brainy theme by MunifTanjim

# Helpers
function __pb13k_remove_empty_elements {
...
function __pb13k_format_duration {
  local duration=$1
...
function __pb13k_timer_start {
  __pb13k_timer=${__pb13k_timer:-$SECONDS}
}

function __pb13k_timer_stop {
  __pb13k_timer_show=$((SECONDS - __pb13k_timer))
  unset -v __pb13k_timer
}

trap '__pb13k_timer_start' DEBUG

# Parsers
function __pb13k_top_left_parse {
  local args
...
function __pb13k_top_right_parse {
  local args
  _omb_util_split args "$1" '|'
  __TOP_RIGHT+=" "
  if [[ ${args[3]} ]]; then
...
function __pb13k_top {
  ...
  local segments
  _omb_util_split segments "$__PB13K_TOP_LEFT"
  local seg info
  for seg in "${segments[@]}"; do
    info=$(__pb13k_prompt_"$seg")
    [[ $info ]] && __pb13k_top_left_parse "$info"
  done
  ...
  _omb_util_split segments "$__PB13K_TOP_RIGHT"
  for seg in "${segments[@]}"; do
    info=$(__pb13k_prompt_"$seg")
    [[ $info ]] && __pb13k_top_right_parse "$info"
  done
...
function __pb13k_bottom {
  local seg segments info
  local __BOTTOM=""
  _omb_util_split segments "$__PB13K_BOTTOM"
  for seg in "${segments[@]}"; do
    info=$(__pb13k_prompt_"$seg")
    [[ $info ]] && __pb13k_bottom_parse "$info"
  done
  printf "\n%s" "$__BOTTOM"
}

# Segments
function __pb13k_prompt_user_info {
  ...
  elif [[ $__PB13K_PROMPT_LOCAL_USER_INFO == true ]]; then
    printf "%s|%s" "$color" "$info"
  fi
}

function __pb13k_prompt_dir {
  local color=$_omb_prompt_bold_navy
  local box=""
  # Cambia \w por \W para que solo muestre el directorio actual
  # Default Config
  # local info="  \w"
  local info="  \W"
  printf "%s|%s|%s|%s" "$color" "$info" "$_omb_prompt_bold_white" "$box"
...
function __pb13k_prompt_python {
  ...
  if ((${#response_array[@]} > 1)); then
    ...
    if [[ "$info" == "py-orden py-«python»,py-orden" ]]; then
      return 0
    fi
...
function __pb13k_prompt_char {
  ...
  printf "%s|%s" "$color" "$__PB13K_PROMPT_CHAR_PS1"
}

function __pb13k_prompt_cmd_duration {
  local color=$_omb_prompt_bold_navy
  local box=""
  local info=$(
    if ((__pb13k_timer_show > 0)); then
      printf " %s" "$(__pb13k_format_duration "$__pb13k_timer_show")"
    fi
  )
  [[ $info ]] || return 0
  printf "%s|%s|%s|%s" "$color" "$info" "$_omb_prompt_bold_white" "$box"
...
# cli
function __pb13k_show {
  local seg=${1:-}
  export "THEME_SHOW_$seg=true"
}

function __pb13k_hide {
  local seg=${1:-}
  export "THEME_SHOW_$seg=false"
}

function __pb13k_completion {
  COMPREPLY=()
...
function pb13k {
  ...
  hide)
    ...
    ;;
  *)
    printf 'pb13k: %s: unrecognized action\n' "$action" >&2
    return 1
    ;;
  esac
...
complete -F __pb13k_completion pb13k
...
__PB13K_PROMPT_CHAR_PS1=${THEME_PROMPT_CHAR_PS1:-"❯"}
__PB13K_PROMPT_CHAR_PS2=${THEME_PROMPT_CHAR_PS2:-"\\"}

__PB13K_TOP_LEFT=${__PB13K_TOP_LEFT:-"dir scm"}
__PB13K_TOP_RIGHT=${__PB13K_TOP_RIGHT:-"exitcode cmd_duration user_info python ruby todo clock battery"}
__PB13K_BOTTOM=${__PB13K_BOTTOM:-"char"}
__PB13K_PROMPT_LOCAL_USER_INFO=${__PB13K_PROMPT_LOCAL_USER_INFO:-"true"}

# Prompt
function __pb13k_ps1 {
  printf "%s%s%s" "$(__pb13k_top)" "$(__pb13k_bottom)" "$_omb_prompt_normal"
}

function __pb13k_ps2 {
  local color
  color=$_omb_prompt_bold_white
  printf "%s%s%s" "$color" "$__PB13K_PROMPT_CHAR_PS2  " "$_omb_prompt_normal"
}
...
_omb_util_add_prompt_command __pb13k_timer_stop
